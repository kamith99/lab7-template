#!/usr/bin/env python3
# Student ID: bkamith@myseneca.ca
class Time:
...
def time_to_sec(time):
    '''Convert a time object to a single integer representing seconds since midnight'''
    minutes = time.hour * 60 + time.minute
    seconds = minutes * 60 + time.second
    return seconds

def sec_to_time(seconds):
    '''Convert an integer (seconds since midnight) into a time object'''
    time = Time()
